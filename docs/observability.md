...
Every `AgentRunner` run has a generated `run_id`, `trace_id`, monotonically increasing event sequence, and timestamp. Attach `SqliteEventSink` from `llama-harness-observability` as the core runner's `EventSink` to persist that causal event stream locally. The sink uses a project-selected SQLite database; it does not start a service or require the developer console to remain open.
