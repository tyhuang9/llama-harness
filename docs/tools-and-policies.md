...
Only tool ID/name, namespace, and aliases enter the cached index. Descriptions, schemas, prompts, provider data, and request data are never indexed or cached as queries/results. Discovery events contain only scope-local candidate/selection counts and whether the catalog exceeded its budget. Cache state remains internal so one caller cannot infer activity from another caller or from tools outside its authorized scope. The default host budget is 64 tools and 128 KiB of exactly serialized tool-definition array data; use `AgentRunner::builder(...).discovery_limits(...)` to lower it. Effective count and byte budgets are the minimum of host and provider limits. Explicit provider capacity of zero follows the normal no-tool model path, while a nonzero budget too small for mandatory hot or exact matches fails before a model or tool call.

Registration bounds the indexed attack surface before compiling schemas: tool IDs are canonical lowercase ASCII with at most 256 bytes, names are canonical printable ASCII with at most 256 bytes, namespaces and aliases are stable lowercase ASCII identifiers with at most 128 bytes each, and each tool can have at most 32 aliases and 4 KiB of aggregate serialized safe discovery metadata. Invalid registration leaves the previous catalog fingerprint and warmed cache unchanged.
