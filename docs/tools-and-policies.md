...
Only tool ID/name, namespace, and aliases enter the bounded metadata index. Descriptions, schemas, prompts, provider data, and request data are never indexed or cached as queries/results. Each lazy index entry is keyed by registry generation, caller, and canonical authorized IDs, so another caller cannot warm it and unallowlisted tools are not traversed during its construction. Exact immutable serialized definition fragments are created once at successful registration and reused for byte accounting and selected-array assembly. Discovery events contain only scope-local candidate/selection counts and whether the catalog exceeded its budget; cache state remains internal. The default host budget is 64 tools and 128 KiB of exactly serialized tool-definition array data; use `AgentRunner::builder(...).discovery_limits(...)` to lower it. Effective count and byte budgets are the minimum of host and provider limits. Explicit provider capacity of zero follows the normal no-tool model path, while a nonzero budget too small for mandatory hot or exact matches fails before a model or tool call.
