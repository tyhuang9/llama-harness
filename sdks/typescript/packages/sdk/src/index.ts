...
export interface ProviderModel { id: string; capabilities: ProviderModelCapabilities; limits?: ProviderModelLimits; }
...
export class HarnessClient {
  ...
  async listModels(): Promise<ProviderModel[]> {
    ...
    return models.map((value) => {
      ...
      if (typeof capabilities?.programmatic_conformance === "string") providerModel.capabilities.programmaticConformance = capabilities.programmatic_conformance as ProgrammaticConformance;
...
function fromWireTool(tool: Record<string, Json>): ToolDefinition { return { id: String(tool.id), name: String(tool.name), description: String(tool.description), argumentsSchema: tool.arguments_schema ?? {}, risk: String(tool.risk) as ToolDefinition["risk"], idempotent: Boolean(tool.idempotent), readOnly: Boolean(tool.read_only), outputSchema: tool.output_schema, parallelSafe: optionalBoolean(tool.parallel_safe), concurrencyKey: typeof tool.concurrency_key === "string" ? tool.concurrency_key : undefined, cancellationSafety: typeof tool.cancellation_safety === "string" ? tool.cancellation_safety as CancellationSafety : undefined, expectedLatencyMs: optionalNumber(tool.expected_latency_ms), allowedCallers: Array.isArray(tool.allowed_callers) ? tool.allowed_callers.filter((value): value is ToolCaller => typeof value === "string") : undefined, speculationPolicy: typeof tool.speculation_policy === "string" ? tool.speculation_policy as SpeculationPolicy : undefined, issueSafety: typeof tool.issue_safety === "string" ? tool.issue_safety as IssueSafety : undefined, executionLocation: typeof tool.execution_location === "string" ? tool.execution_location as ExecutionLocation : undefined, networkEgress: typeof tool.network_egress === "string" ? tool.network_egress as NetworkEgress : undefined }; }
